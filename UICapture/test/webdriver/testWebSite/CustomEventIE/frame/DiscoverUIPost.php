    ...
    function ProcessPost()
    {
      if (!strcasecmp($_SERVER['REQUEST_METHOD'], "POST")) {
        ...
        try {
		  ...
		  //$postFileHandle = fopen("C://xampp//htdocs//AndroidTest//raw.txt", 'w+');
          if ($postFileHandle) {
            ...
            if ($postData) {
              $actualReadLength = strlen($postData);
				...
				if( isset( $headers['X-Discover'] )  &&  !strcasecmp( $headers['Content-Type'] , 'application/json; charset=UTF-8' ) ) {
					  $writeFile = fopen("raw.txt" , 'w+' );
					  fwrite($writeFile , $postData);
					  fclose($writeFile);
				}
            }
            else {
...
<html>
<head><title>Discover Target</title></head>
<body>
<?php
      echo "\r\nDiscover Target $DCX_TARGET_VERSION\r\n<br />\r\n";
      echo ProcessPost();
