	...
	if (os.path.isfile(lcov_path)):
		...
		for line in lcov_file:
			...
			if input_type == 'SF':
				...
				src_name = os.path.realpath(file_name)
...
def generate_cobertura_xml(coverage_data, options):
	...
	sources_element = document.createElement('sources')
	packages_element = document.createElement('packages')

	source_element = document.createElement('source')
	source_element.appendChild(document.createTextNode(os.path.realpath(options.base_dir)))
	...
	root.appendChild(sources_element)
