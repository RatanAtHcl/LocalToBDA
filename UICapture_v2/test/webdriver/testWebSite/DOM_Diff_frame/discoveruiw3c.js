    ...
    function flushQueue(queueId, sync) {
        var data = queueManager.flush(queueId),
            ...
            httpHeaders = {
                ...
                "X-Discover": "device (UIC) Lib/5.0.0.1688",
...
DCX.addService("domCapture", function (core) {
    ...
    function isNodePartOfMutatedTargets(xpath, node) {
        ...
        for (i = 0, len = mutatedTargets.length; !found && i < len; i += 1) {
            ...
            if (xpath.containedIn(target)) {
                ...
                for (j = 0; j < target.addedNodes.length; j += 1) {
                    if (target.addedNodes[j].contains(node)) {
                        found = true;
                        break;
                    }
                }
    ...
    function getDOMDiff(options) {
        ...
        for (i = 0, len = mutatedTargets.length; i < len; i += 1) {
            ...
            dc = getDOMCapture(window.document, target, targetXpath, options);
            diff = {
    ...
    return {
        ...
        captureDOM: function (root, options) {
            ...
            if (!fullDOMSent || !diffEnabled || options.forceFullDOM) {
                if (diffObserver) {
                    // Stop observing
                    diffObserver.disconnect();
                }
...
DCX.addService("message", function (core) {
    ...
    function matchesTarget(targets, target) {
        ...
        for (i = 0, len = targets.length; i < len; i += 1) {
            ...
            } else if (tmpTarget.id && tmpTarget.idType && target.idType === tmpTarget.idType) {
...
DCX.addModule("DCCookie", function (context) {
    ...
    function createDCXSID() {
        var cookieValue = random32LengthString(),
            locationArray = location.hostname.split('.'),
            i;

        // Set the session cookie
        //Try to set the cookie with two domain components. e.g. "ibm.com". If not successful try with three domain components, e.g. "ibm.co.uk" and so on.
        for (i = 2; i <= locationArray.length; i += 1) {
            document.cookie = sessionCookieName + "=" + cookieValue + ";domain=" + locationArray.slice(-i).join('.') + ";path=/";
            if (utils.getCookieValue(sessionCookieName) === cookieValue) {
                return cookieValue;
            }
        }
        return "";
    ...
    function processConfig(config) {
        ...
        if (config.sessionizationCookieName) {
            ...
            if (!sessionCookieValue && sessionCookieName === "TLTSID") {
                ...
                sessionCookieValue = createDCXSID();
            }
            reqHeaders.push(
                {
...
DCX.addModule("replay", function (context) {
    ...
    function createQueueEvent(options) {
        var control,
            ...
            dcType        = dcTypes[targetType.toLowerCase() + ":" + targetSubtype.toLowerCase()] || targetType,
    ...
    function getRelativeXY(webEvent) {
        ...
        node = node && node.element ? node.element : node;
        cellWidth = node.offsetWidth > 0 ? Math.max(node.offsetWidth / gridValues.cellMaxX, gridValues.cellMinWidth) : gridValues.cellMinWidth;
        cellHeight = node.offsetHeight > 0 ? Math.max(node.offsetHeight / gridValues.cellMaxY, gridValues.cellMinHeight) : gridValues.cellMinHeight;
        cellX = Math.floor(offsetX / cellWidth);
        cellY = Math.floor(offsetY / cellHeight);
        ...
        services: {
            queue: {
                ...
                queues: [
                    {
                        qid: "DEFAULT",
                        endpoint: "/webapp/wcs/stores/Aurora/DiscoverUIPost.jsp",
                        maxEvents: 50,
