            ...
            if (queues[i].checkEndpoint) {
                ajaxService.sendRequest({
                    oncomplete: function (result) {
                        //do nothing
                        if (result.success) {
                            console.log("Endpoint check passed: ", result);
                        }
                    },
                    ...
                    error: function (result) {
                        console.log("Endpoint check failed: ", result);
...
/*jshint devel:true */
    ...
    function handleXhrCallback(result) {
        ...
        core.utils.clog("XHR Result: ", result);
    }
    ...
    function flushQueue(queueId, sync) {
        var data = queueManager.flush(queueId),
            ...
            httpHeaders = {
                ...
                "X-Discover": "device (UIC) Lib/5.2.0.1766",
                "X-DiscoverType": "GUI",
        ...
        if (timeDiff > (inactivityTimeout + 60000)) {
            core.utils.clog("UIC self-terminated due to inactivity(1).");
            return;
        }
    ...
    function addToQueue(queueId, data) {
        ...
        if (len) {
            ...
            if (timeDiff > inactivityTimeout) {
                ...
                core.utils.clog("UIC self-terminated due to inactivity(2).");
                core.destroy();
                return;
            }
        }
...
if (DCX && typeof DCX.addModule === "function") {
    ...
    DCX.addModule("usability", function (context) {
        ...
        function postUIEvent(hoverEvent, options) {
            ...
            utils.clog("Usability - posted hover event");
            utils.clog(uiEvent);
        ...
        function calculateNodeOffset(node) {
            ...
            var boundingRect = node && node.getBoundingClientRect ? node.getBoundingClientRect() : null,
                offsetX =  boundingRect ? boundRect.left : (node ? node.offsetLeft : 0),
                offsetY = boundingRect ? boundRect.top : (node ? node.offsetHeight : 0),
                lastOffsetX = offsetX,
                lastOffsetY = offsetY,
                offsetDiffX = 0,
                offsetDiffY = 0,
        ...
        function calculateGridCell(node, offsetX, offsetY) {
            ...
            var boundingRect = node.getBoundingClientRect ? node.getBoundingClientRect() : null,
                oWidth =  boundingRect ? boundRect.width : node.offsetWidth,
                oHeight = boundingRect ? boundRect.height : node.offsetHeight,
